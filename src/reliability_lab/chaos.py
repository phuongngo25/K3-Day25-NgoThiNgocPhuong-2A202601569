    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                recovery_deltas_ms.append((float(entry["ts"]) - open_ts) * 1000)
                open_ts = None

    if not recovery_deltas_ms:
        return None
    return sum(recovery_deltas_ms) / len(recovery_deltas_ms)
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        # Pass/fail criteria per scenario, scaled to how many providers are
        # degraded: a healthy baseline must serve almost everything; a single
        # degraded provider must be masked by fallback; when both providers
        # are degraded together we only require the system to stay partially
        # available (cache + residual backup capacity + fast static fallback)
        # rather than demanding full fallback success, since some requests
        # are expected to legitimately hit the static-fallback path.
